...
public class WavParam {

    int simpleRate;
    private Format format;

    WavParam(int simpleRate, Format f) {
        this.simpleRate = simpleRate;
        this.format = f;
    }

    WavParam(int simpleRate, int channelCount, int bits) {
        this.simpleRate = simpleRate;
    ...
    int getChannelCount() {
        return (format == Format.SINGLE_8_BIT || format == Format.SINGLE_16_BIT) ? 1 : 2;
    }
